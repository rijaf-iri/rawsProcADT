...
initializeAWSStatus <- function(aws_dir){
    ...
    last0 <- round.POSIXt(last, units = "hours")
    if(last0 - last < 0){
        last <- last0 + 3600
    }else{
        last <- last0
    ...
    update_actual <- format(last, "%Y%m%d%H")
...
get_status_percentage <- function(time_obs, time_ts){
    ## Time step for each aws read from external table
    # n <- unique(qres$network[i])
    # timestep <- tstep[n]

    ## Time step for each aws computed each time
    df <- diff(time_obs)
